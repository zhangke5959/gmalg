...
static int public_block_operation(u8 *out, u32 *out_len, u8 *in, u32 in_len, rsa_pk_t *pk);
static int private_block_operation(u8 *out, u32 *out_len, u8 *in, u32 in_len, rsa_sk_t *sk);
...
static void vli_decode(u64 *bn, u32 digits, u8 *hexarr, u32 size)
{
	u64 t;
	u32 i, u;
	int j;
...
static void vli_encode(u8 *hexarr, u32 size, u64 *bn, u32 digits)
{
	u64 t;
	u32 i, u;
	int j;
...
static int public_block_operation(u8 *out, u32 *out_len, u8 *in, u32 in_len, rsa_pk_t *pk)
{
	...
	vli_decode(m, RSA_MAX_DIGITS, in, in_len);
	vli_decode(n, RSA_MAX_DIGITS, pk->modulus, RSA_MAX_MODULUS_LEN);
	vli_decode(e, RSA_MAX_DIGITS, pk->exponent, RSA_MAX_MODULUS_LEN);
	...
	*out_len = bytes; 
	vli_encode(out, bytes, c, RSA_MAX_DIGITS);
...
static int private_block_operation(u8 *out, u32 *out_len, u8 *in, u32 in_len, rsa_sk_t *sk)
{
	...
	vli_decode(c, RSA_MAX_DIGITS, in, in_len);
	vli_decode(c_, RSA_MAX_DIGITS, in, in_len);
	vli_decode(n, RSA_MAX_DIGITS, sk->modulus, RSA_MAX_MODULUS_LEN);
	vli_decode(p, RSA_MAX_DIGITS, sk->prime1, RSA_MAX_PRIME_LEN);
	vli_decode(q, RSA_MAX_DIGITS, sk->prime2, RSA_MAX_PRIME_LEN);
	vli_decode(dp, RSA_MAX_DIGITS, sk->prime_exponent1, RSA_MAX_PRIME_LEN);
	vli_decode(dq, RSA_MAX_DIGITS, sk->prime_exponent2, RSA_MAX_PRIME_LEN);
	vli_decode(q_inv, RSA_MAX_DIGITS, sk->coefficient, RSA_MAX_PRIME_LEN);
